...
describe('ContractProvider', () => {
  ...
  let mockJsonRpcProvider: jest.Mock;
  let mockContract: jest.Mock;

  beforeEach(() => {
    vi.resetAllMocks();
    mockJsonRpcProvider = JsonRpcProvider as jest.Mock;
    mockContract = Contract as jest.Mock;
  ...
  describe('getContract', () => {
    it('should return a contract instance', async () => {
      ...
      provider.registry.addContract(mockContractDef);
  ...
  describe('registry', () => {
    it('should add and retrieve contracts', () => {
      const mockContractDef: IContractDefinition = {
        name: 'TestContract',
        address: '0x1234567890123456789012345678901234567890',
        chainId: 1,
      ...
      provider.registry.addContract(mockContractDef);
      const retrievedContract = provider.registry.getContract('TestContract');

      expect(retrievedContract).toEqual(mockContractDef);
    });

    it('should return undefined for non-existent contracts', () => {
      const retrievedContract = provider.registry.getContract('NonExistentContract');
